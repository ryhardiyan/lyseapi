  ...
  ogTags.forEach(({ property, content }) => {
    ...
    async function loadSettings() {
      ...
      const settings = await fetch("/settings.json").then(res => res.json());
      const apis = await fetch("/api/endpoints.json").then(res => res.json());
allAPIs = apis; // Simpan semua data
document.getElementById("searchInput").addEventListener("input", function () {
  ...
  // Tampilkan hasil pencarian gabungan semua kategori
...
function fetchAPI(endpoint) {
  ...
  document.querySelectorAll('.hotbar button').forEach((btn, i) => {
  btn.addEventListener('click', () => {
    alert(`Hotbar tombol ${i + 1} diklik`);
  });
});
}